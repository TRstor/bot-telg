...
// دالة لحساب hash للـ URL (لاستخدامها كـ doc ID)
      ...
      try {
        ...
        if (!existingSnapshot.empty) {
          ...
          if (existingData.storageUrl) {
            skipped++;
            if (processed % 50 === 0) {
              console.log(`📊 معالجة: ${processed} (نقل: ${count}, تخطي: ${skipped})`);
            }
            continue; // موجودة بالفعل في Storage
          }

          // تحديث الصورة الموجودة برابط Storage الجديد
          const storageUrl = await uploadImageToStorage(ibbUrl, `img-${count}.jpg`);
...
// إضافة صورة إلى المفضلات
async function addToFavorites(userId, imageUrl, imageName) {
  const result = initializeFirebase();
  ...
  try {
    ...
    console.log(`❤️ تمت إضافة "${imageName}" إلى المفضلات`);
    return true;
  } catch (err) {
    console.error('❌ خطأ في إضافة للمفضلات:', err.message);
    return false;
  }
}

// حذف صورة من المفضلات
async function removeFromFavorites(userId, imageUrl) {
  const result = initializeFirebase();
  ...
  try {
    ...
    console.log(`💔 تمت إزالة الصورة من المفضلات`);
    return true;
  } catch (err) {
    console.error('❌ خطأ في حذف من المفضلات:', err.message);
    return false;
  }
}
...
async function getFavorites(userId) {
  ...
  try {
    ...
    console.log(`❤️ تم تحميل ${favorites.length} مفضلة`);
    return favorites;
  } catch (err) {
    console.error('❌ خطأ في جلب المفضلات:', err.message);
    return [];
  }
}
...
// تسجيل عملية بحث
async function logSearch(userId, searchQuery, resultsCount, category = null) {
  const result = initializeFirebase();
  ...
  try {
    ...
    console.error('❌ خطأ في تسجيل البحث:', err.message);
    return false;
  }
}
...
// تحديث الإحصائيات
async function updateImageStatistics(imageUrl, action = 'view') {
  const result = initializeFirebase();
  ...
  try {
    // استخدام hash للـ URL كـ doc ID بدلاً من الـ URL نفسه
    const urlHash = getUrlHash(imageUrl);
    const docRef = db.collection('statistics').doc(urlHash);
    ...
    if (action === 'view') {
      ...
      await docRef.update({
        ...
        // تجاهل الخطأ إذا لم توجد الإحصائية
      });
    }
